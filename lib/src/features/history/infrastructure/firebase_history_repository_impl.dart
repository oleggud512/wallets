...
class FirebaseHistoryRepositoryImpl implements HistoryRepository {
  ...
  Future<void> deleteHistoryNode(String walletId, String nodeId) async {
    await source.userRef
        .child(FirebaseStrings.historyNode(walletId, nodeId))
        .remove();
  }

  @override
  Future<void> makeTransaction(
      String walletId, HistoryNode transactionData) async {
    await source.userRef
        .child(FirebaseStrings.wallet(walletId))
        .runTransaction((Object? wallet) {
      ...
      if (transactionData.action == WalletAction.add) {
        ...
        if (double.parse(walMap[FirebaseStrings.amount].toString()) <
      ...
      walMap[FirebaseStrings.lastUpdated] =
          transactionData.date.millisecondsSinceEpoch;
      return Transaction.success(walMap);
    });

    await source.userRef
        .child(FirebaseStrings.historyOf(walletId))
  ...
  Future<void> updateHistoryNodeDescription(
      String wid, String hid, String newDescription) async {
    await source.userRef
        .child(FirebaseStrings.historyNodeDescription(wid, hid))
        .set(newDescription);
  }
}
